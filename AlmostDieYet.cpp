...
void equipment ()
{
	while (bool keeppocketopen=true) {
		printf ("HP: %d\t\t\tCoin: %d\n", HP, coin);
		int count=0;
		if (bag[0][0]) printf ("Sword\t");
		if (bag[0][1]) printf ("Long Sword\t");
		if (bag[0][2]) printf ("Bow\t");
		if (bag[0][3]) printf ("Dragger\t");
		printf("\n\n");
		puts ("0. Confirm\n1. Ehh...\nClose pocket? ");
		int option;
		scanf ("%d", &option);
...
void food ()
{
	while (bool keeppocketopen=true) {
		...
		int count=0, *temp[20];
			...
			if (bag[1][10]) {printf ("%d. %17s: %d     ", count, "Cooked small fish", bag[1][10]); temp[count]=&bag[1][10]; count++;}
			if (bag[1][11]) {printf ("%d. %18s: %d    ", count, "Cooked medium fish", bag[1][11]); temp[count]=&bag[1][11]; count++;}
			if (bag[1][12]) {printf ("%d. %17s: %d     ", count, "Cooked large fish", bag[1][12]); temp[count]=&bag[1][12]; count++;} puts("");
			if (bag[1][13]) {printf ("%d. %11s: %d     ", count, "Cooked meat", bag[1][13]); temp[count]=&bag[1][13]; count++;}
			if (bag[1][14]) {printf ("%d. %13s: %d     ", count, "Bacon and egg", bag[1][14]); temp[count]=&bag[1][14]; count++;}
			if (bag[1][15]) {printf ("%d. %10s: %d     ", count, "Fried eggs", bag[1][15]); temp[count]=&bag[1][15]; count++;}
		for (int indicator=count; indicator<20; indicator++) temp[indicator]=0;
		printf("\n\n");
		...
		if (temp[option]==&bag[1][9]) {
			bag[1][9]--;
			puts ("You just ate an uncooked egg?"); sleep(2); puts ("So boid"); sleep(2); puts ("+3 HP"); sleep(2); HP+=3; printf ("\e[1;1H\e[2J");
			if (HP>100) HP=100;
		}
		if (temp[option]==&bag[1][10]) {
			bag[1][10]--;
			puts("You ate cooked small fish"); sleep(2); puts("+4 HP"); sleep(2); printf ("\e[1;1H\e[2J"); HP+=4; if (HP>100) HP=100;
		}
		if (temp[option]==&bag[1][11]) {
			bag[1][11]--;
			int consume=rand()%2+5;
			puts("You ate cooked medium fish"); sleep(2); printf("+%d HP", consume); sleep(2); printf ("\e[1;1H\e[2J"); HP+=consume; if (HP>100) HP=100;
		}
		if (temp[option]==&bag[1][12]) {
			bag[1][12]--;
			int consume=rand()%2+7;
			puts("Cooked large fish is delicious"); sleep(2); printf("+%d HP", consume); sleep(2); printf ("\e[1;1H\e[2J"); HP+=consume; if (HP>100) HP=100;
		}
		if (temp[option]==&bag[1][13]) {
			bag[1][13]--;
			puts("You just ate cooked meat or a steak?"); sleep(2); puts("+7 HP"); sleep(2); printf ("\e[1;1H\e[2J"); HP+=7; if (HP>100) HP=100;
		}
		if (temp[option]==&bag[1][14]) {
			bag[1][14]--;
			puts("Well-cooked bacon and egg"); sleep(2); puts("+12 HP"); sleep(2); printf ("\e[1;1H\e[2J"); HP+=12; if (HP>100) HP=100;
		}
		if (temp[option]==&bag[1][15]) {
			bag[1][15]--;
			puts("Fried eggs are good for health"); sleep(2); puts("+7 HP"); sleep(2); printf ("\e[1;1H\e[2J"); HP+=7; if (HP>100) HP=100;
		}
		if (option==count) {
			printf ("0. Confirm\n1. Ehh...\nClose pocket?\n");
			scanf ("%d", &option);
...
void goods (int &enemyHP)
{
	...
	while (keeppocketopen) {
		...
		int count=0, *temp[10];
			...
			for (int indicator=count; indicator<10; indicator++) temp[indicator]=0;
		printf("\n\n");
		if (enemyHP>0) {
...
void cook ()
{
	while (bool cooking=true) {
		...
		for (int indicator=count; indicator<10; indicator++) strcpy(temp[indicator], "Nothing");
		puts ("");
		printf ("%d. Nothing\n", count);
		int option;
		...
		if (option==1) {
			...
			if (temp==0) puts ("The rat jump to bite you but you block it");
					...
					if (temp==0) {
						...
						if (temp==0) {
							sleep(2);
							puts("It quickly bite you third time"); sleep(2);
							temp=rand()%2+1;
							printf ("-%d HP\n", temp); HP-=temp; if (HP<=0) {sleep(2); HP=0; break;}
						}
					}
					...
					if (bag[1][0]||bag[1][1]||bag[1][6]) {
						...
						while (bool keeprandom=true) {
							...
							if (steal[eat]=&bag[1][0]) {
								...
								else {consume=rand()%3+5; puts("Juicy apple"); sleep(2); printf("Monkey +%d HP", consume); sleep(2); monkeyHP+=consume; if (monkeyHP>40) monkeyHP=40;}
								break;
								...
								else {consume=rand()%2+2; puts("Fresh berry"); sleep(2); printf("Monkey +%d HP", consume); sleep(2); monkeyHP+=consume; if (monkeyHP>40) monkeyHP=40;}
								break;
								...
								else {consume=rand()%2+3; puts("Yum banana"); sleep(2); printf("Monkey +%d HP", consume); sleep(2); monkeyHP+=consume; if (monkeyHP>40) monkeyHP=40;}
								break;
							}
						}
					}
		...
		if (temp<2) {
			...
			if (bag[1][2]>0||bag[1][3]>0||bag[1][4]>0) {
				...
				while (bag[1][2]>0||bag[1][3]>0||bag[1][4]>0) {
					...
					for (int indicator=count; indicator<3; indicator++) sell[indicator]=0;
					puts ("");
	...
	while (bool keepevent=true) {
		...
		if (option==3) {
			...
			if (temp<7) {
				...
				puts ("You take the chance to cook something"); sleep(5);
				printf ("\e[1;1H\e[2J");
				cook ();
			}
		}
...
void BuyWeapon()
{
    ...
    do {
		printf ("HP: %d\t\t\tCoin: %d\n", HP, coin);
		int *temp[3], count=0;
        if (bag[0][1]==0) {printf("%d. LongSword (15 coins to purchase, and it deals extra 5 damage)\n", count); temp[count]=&bag[0][1]; count++;}
        if (bag[0][2]==0) {printf("%d. Bow (10 coins to purchase, and it grants extra 4 damage)\n", count); temp[count]=&bag[0][2]; count++;}
        if (bag[0][3]==0) {printf("%d. The dagger (20 coins to buy, deals extra 7 damage)\n", count); temp[count]=&bag[0][3]; count++;}
        for (int indicator=count; indicator<3; indicator++) temp[indicator]=0;
        printf ("%d. Nothing\n", count);
        printf ("You want to buy ");
        scanf("%d", &choice);
        printf("\e[1;1H\e[2J");
		if (temp[choice]==&bag[0][1])
            if (coin >= 15) {
				bag[0][1]=1; dame += 5; coin -= 15;
                printf("You have purchased a longsword, you can do extra 5 damage");
            } else puts("I'm sorry, but you don't have enough money to complete the transaction");
        if (temp[choice]==&bag[0][2])
            if (coin >= 10) {
                bag[0][2]=1; dame += 4; coin -= 10;
                printf("You choose to buy a wooden bow and be able to do extra 4 damage");
            } else puts("I'm sorry, but you don't have enough money to complete the transaction");
        if (temp[choice]==&bag[0][3])
            if (coin >= 20) {
                bag[0][3]=1; dame += 7; coin -= 20;
                printf("You choose to buy the dagger. You can do extra 7 damage");
            } else puts("I'm sorry, but you don't have enough money to complete the transaction.\n");
        sleep (3); printf("\e[1;1H\e[2J");
...
void buyequipment()
{
	...
	do {
    	printf ("HP: %d\t\t\tCoin: %d\n", HP, coin);
        puts("1. Health Potion (Cost: 5 coins, Restores 10 HP.)");
        puts("2. Lucky Dice (Cost: 5 coins, 5% chance to receive 10 coins.)");
        puts("3. Large Healing Potion (Cost: 35 coins, 3% chance to add 100 HP.)");
        printf("0. Nothing\nYou want to buy ");
        scanf("%d", &choice);
        printf("\e[1;1H\e[2J");
        if (choice==1)
			if (coin >= 5)
				if (HP < 100) {
					HP += 10; coin -= 5; if (HP>100) HP=100;
					printf("You bought a Health Potion. It restores 10 HP");
				} else puts("Your HP is already full. No need to purchase a Health Potion");
			else puts("Sorry, you don't have enough coins to complete the transaction");
		if (choice==2)
            if (coin >= 5) {
                coin -= 5;
				if (rand() % 100 < 5) {
					coin += 10;
                    printf("You bought a Lucky Dice. You received 10 coins");
                    } else puts("You bought a Lucky Dice. Unfortunately, you didn't win any coins");
                } else puts("Sorry, you don't have enough coins to complete the transaction");
        if (choice==3)
            if (coin >= 35)
                if (HP < 100) {
                    coin -= 35;
                    if (rand() % 100 < 3) {
                        HP += 100; if (HP > 100) HP = 100;
                        printf("You bought a Large Healing Potion. It added 100 HP");
                    } else {
						puts("You bought a Large Healing Potion. Unfortunately, it doesn't have the extra healing effect and only restores 60 HP");
						HP += 60; if (HP > 100) HP = 100;
                    }
            	} else puts("Your HP is already full. No need to purchase a Large Healing Potion.");
            else puts("Sorry, you don't have enough coins to complete the transaction.");
        sleep (3); printf("\e[1;1H\e[2J");
        if (choice==0) break;
    } while (true);
...
void Merchant(int &eventcount)
{
	...
	puts("The old man is a merchant and he invites you to buy his goods");sleep(5);
	printf ("\e[1;1H\e[2J");
    while (true) {
	printf ("HP: %d\t\t\tCoin: %d\n", HP, coin);
	printf("Welcome to Merchant's shop!\n");sleep(1);
	puts("0. Open bag\n");sleep(1);
	puts("1. Buy Weapon\n");sleep(1);
	puts("2. Buy Equiment\n");sleep(1);
	puts("3. Exit\n");sleep(2);
	...
	if (choice==0) {int blockusegoods=0; openbag (blockusegoods);}
	printf ("\e[1;1H\e[2J");
    }
}
...
void river (int &eventcount)
{
	...
	while (eventcount<5) {
		int temp=rand()%7+1;
		if (blockrepeat[0]==temp||blockrepeat[1]==temp||blockrepeat[2]==temp||blockrepeat[3]==temp||blockrepeat[4]==temp) continue;
		...
		if (temp==6) unexpectedflood (eventcount);
		if (temp==7) if (rand()%3==0) Merchant (eventcount);
		if (HP==0) break;
	}
}
...
void forest (int &eventcount)
{
	...
	while (eventcount<2) {
		...
		if (temp==3) if (rand()%3==0)Merchant(eventcount);
		if (HP==0) break;
	}
}
...
void process (int &HP)
{
	...
	if (HP==0) {
		...
		printf ("\e[1;1H\e[2J");
		if (option==1) {
			HP=100; daycount=0; coin=0; dame=0;
			for (int i=0; i<4; i++)
